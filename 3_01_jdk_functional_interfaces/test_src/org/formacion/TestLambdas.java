...
public class TestLambdas {
	...
	public void test_extras() {
		...
		Predicate<Persona> sinSegundoApellido = (persona) -> null == persona.getApellido2() ? true : false; 
		
		assertEquals(true, sinSegundoApellido.test(personaSinSegundoApellido));
		assertEquals(false, sinSegundoApellido.test(personaConSegundoApellido));
		
		// Una funcion que nos diga si dos personas son parientes: para nosotros parientes
		// son personas con el mismo primer apellido
		BiPredicate<Persona, Persona> sonFamilia = (p1, p2) -> p1.getApellido1().equals(p2.getApellido1()) ? true : false;
		
		assertEquals(true, sonFamilia.test(personaSinSegundoApellido, personaConSegundoApellido));
		assertEquals(false, sonFamilia.test(personaSinSegundoApellido, personaNoPariente));
		assertEquals(false, sonFamilia.test(personaConSegundoApellido, personaNoPariente));
	...
	public void test_validadores() {
		...
		validador.add((p) -> p.getApellido1() == null ? false : true);
